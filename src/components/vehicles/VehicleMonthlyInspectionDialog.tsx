...
export function MonthlyInspectionDialog({
  ...
  const [lastReports, setLastReports] = useState<VehicleMonthlyInspection[]>([]);
  ...
  const handleSubmit = async () => {
    try {
      ...
      const existingReports = JSON.parse(localStorage.getItem(`vehicle_monthly_inspection_reports_${vehicle.id}`) || '[]');
      localStorage.setItem(
        `vehicle_monthly_inspection_reports_${vehicle.id}`,
  ...
  async function fetchLastInspectionReports(vehicleId: string): Promise<VehicleMonthlyInspection[]> {
    const storedReports = JSON.parse(localStorage.getItem(`vehicle_monthly_inspection_reports_${vehicleId}`) || '[]');
      ...
      <Dialog open={open} onOpenChange={onOpenChange}>
        <DialogContent className="w-[95vw] max-w-[1200px] max-h-[90vh] overflow-y-auto p-6 rounded-xl shadow-2xl bg-white">
          ...
          <div className="bg-gray-50 p-4 rounded-lg mb-6 shadow-sm">
            <h3 className="font-semibold text-lg mb-3 text-gray-800">MONTHLY VEHICLE INSPECTION CHECKLIST</h3>
          ...
          <div className="flex justify-end gap-2 pt-4 border-t">
            <Button variant="outline" onClick={() => window.print()} className="hover:bg-gray-100 transition-all">
              ...
              {isRTL ? "Print Report" : "Print Report"}
            </Button>
                ...
                <>
                  ...
                  <div className="bg-gray-50 p-4 rounded-lg mb-6 shadow-sm">
                    ...
                    <div className="grid grid-cols-2 md:grid-cols-3 gap-4 text-sm">
                      ...
                      <div><span className="font-medium">{isRTL ? "المقاول الفرعي:" : "Subcontractor:"}</span> {/* Removed selectedReport.manufacturer */}</div>
                      <div><span className="font-medium">{isRTL ? "رقم الشاسيه:" : "Chassis No:"}</span> {/* Removed selectedReport.serialNumber */}</div>
                  ...
                  <div className="bg-gray-50 p-4 rounded-lg mb-6 shadow-sm">
                    <h3 className="font-semibold text-lg mb-3 text-gray-800">{isRTL ? "معلومات المركبة" : "Vehicle Information"} :</h3>
                    <div className="grid grid-cols-2 md:grid-cols-3 gap-4 text-sm">
                      <div><span className="font-medium">{isRTL ? "اسم المركبة:" : "Vehicle Name:"}</span> {selectedReport.vehicleId || vehicle.name}</div>
                      <div><span className="font-medium">{isRTL ? "رقم اللوحة:" : "Plate Number:"}</span> {vehicle.plateNumber}</div>
                      <div><span className="font-medium">{isRTL ? "المشروع:" : "Project:"}</span> {selectedReport.notes}</div>
                      <div><span className="font-medium">{isRTL ? "عداد المسافات:" : "Mileage:"}</span> {selectedReport.mileage}</div>
                    </div>
        ...
        <DialogContent className="w-[95vw] max-w-[1400px] max-h-[90vh] overflow-y-auto p-6 rounded-xl shadow-2xl bg-white">
          <DialogHeader className="flex flex-row items-center justify-between space-y-0 pb-4 border-b">
            <DialogTitle className="text-2xl font-bold text-gray-800">
              {isRTL ? "الفحص الشهري" : "Monthly Vehicle Inspection Checklist"}
            </DialogTitle>
          ...
          {/* Vehicle and Project Info */}
          <div className="bg-gray-50 p-4 rounded-lg mb-6 shadow-sm">
            <h3 className="text-lg font-semibold mb-3 text-gray-800">
              {isRTL ? "قائمة فحص المركبة" : "Vehicle Inspection Checklist"}
            </h3>
            <p className="text-sm mb-3 text-gray-500">{vehicle.name || "N/A"}</p>
            <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
              ...
              <div>
                <label className="text-sm font-medium text-gray-600">{isRTL ? "عداد المسافات:" : "Mileage:"}</label>
                <Input
                  type="number"
                  value={mileage}
                    ...
                    {(responses[item.id]?.status === "failed" || showCommentBox) && (
                      <div className="space-y-2">
                        <Input
                          ...
                          className="h-8 text-xs border-gray-300 focus:border-blue-500 focus:ring-blue-200"
                        />
                        {responses[item.id]?.status === "failed" && (
                          <Input
                            ...
                            className="h-8 text-xs border-red-200 focus:border-red-300 focus:ring-red-100"
                          />
                        )}
                      </div>
                    )}
